$(document).ready(function(){
    ...
    function startProcess(event){
        ...
        switch(event.keyCode){
            case 13:{                          
                if(abilitaComando(temp, "start", "spacesht")){
                    div.innerHTML = "";
                    styleCssLoading();
                    ...
                    btnStart.href='rules.html';
                    btnStart.innerHTML =
                    "<button> <span> go to </span> </button> ";
                }

                else if(abilitaComando(temp, "start", "s!ns")){
                    div.innerHTML = "";
                    div.appendChild(btnStart);
                    btnStart.href='https://sins.altervista.org/';
                    btnStart.innerHTML =
                    "<button> <span> play </span> </button> ";
                }

                else{
                    confirm("Per avviare le regole scrivere 'rule', per avviare s!ns scrivere 'start s!ns', per avviare space shooter scrivere start spacesht'");
                    div.innerHTML = "";
                }
